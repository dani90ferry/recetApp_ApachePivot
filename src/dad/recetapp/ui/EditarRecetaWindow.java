...
public class EditarRecetaWindow extends Window implements Bindable {
	...
	public void cargarReceta() {
		...
		try {
			...
			if(secciones == null) {
				//Se introduce una pestaña inicial vacía
				initRecetasTab();
			} else {
	...
	protected void onGuardadButtonPressed() {
		try {
			...
			if(categoriaListButton.getSelectedIndex() == 0) {
				...
				//Se vacían las secciones del componente antes de insertar las que hay actualmente
				//Para que no se dupliquen las pestañas
				receta.getSecciones().clear();
				TabSequence tabs = recetasTab.getTabs();
				//getLenght() - 1 para que no tenga en cuenta la pestaña + (Esa es fija)
